...
{
	name = "l4d2 restart without changelevel",
	author = "IA/NanaNana",
...
{
	Handle a = LoadGameConfigFile("l4d2_restart_without_changelevel")

	if(!a)
		SetFailState( "Can't load gamedata \"l4d2_restart_without_changelevel.txt\" or not found" )
	if(!(hRestartDetour = DHookCreateFromConf(a, "CDirector::Restart")))
		SetFailState( "Can't create detour \"CDirector::Restart\"" )
	...
	iPatch1 = GameConfGetAddress(a, "CDirector::Restart") + view_as<Address>(GameConfGetOffset(a, "CDirector::Restart__Patch1"));
	CloseHandle(a)
...
{
	StoreToAddress(a+view_as<Address>(452), 1, NumberType_Int8)
}
